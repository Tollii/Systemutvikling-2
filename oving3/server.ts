...
app.get("/article", (req:any, res:any) => {
  ...
  pool.getConnection((err:any, connection:any) => {
    ...
    if(err){
      ...
      connection.query(
        "SELECT article_id, title, article_text, created_at, image, importance FROM article",
        (err:any, rows:any) => {
          connection.release();
          if(err){
            console.log(err);
...
app.delete("/article", (req:any, res:any) => {
  ...
  pool.getConnection((err:any, connection:any) => {
    if(err){
      ...
      console.log("Established connection to database");
      connection.query(`DELETE FROM article WHERE article_id = ?;`, [req.body.id],
        err => {
          if(err){
            console.log(err);
            res.status(500);
...
app.put("/article", (req:any, res:any) => {
